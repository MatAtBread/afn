...
module.exports = function(opts) {
    if (!opts) opts = {} ;
    opts.Promise = opts.Promise || global.Promise ;
		...
		return new (opts.Promise)(function ($return,$error) {
			...
			array.forEach(function(e,i,ar){
				function complete(r) {
					...
					var k = isArray?i:e ; 
					if (k in result) {
						context.message = "afn.map: multiple $returns/errors at "+k ; 
					...
					if (len==0) {
						...
						context.message = "afn.map: Excess $returns/errors at "+k ;
						return $error(context) ;
					}
				} ;
				...
				if (asyncFn) {
					...
					if (f && f instanceof Object && typeof f.then==="function")
